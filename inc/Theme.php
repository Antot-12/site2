...
class Theme {
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'streamit' ),
						gettype( $component ),
	 ...
	 *
	 * Calling `streamit()` is a short-hand for calling this method on the main theme instance. The instance then allows
	 * for actual template tag methods to be called. For example, if there is a template tag called `posted_on`, it can
	 * be accessed via `streamit()->posted_on()`.
	 *
	...
	public function component( string $slug ) : Component_Interface {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'streamit' ),
					$slug
				)
			);
		}
	...
	protected function get_default_components() : array {
		...
		if (class_exists('MasVideos')) {
			$components[] = new Remove_Hook\Component();
		}
