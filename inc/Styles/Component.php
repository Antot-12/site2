...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function action_enqueue_styles()
	{
		...
		foreach ($css_files as $handle => $data) {
			...
			$version = streamit()->get_asset_version($css_dir . $data['file']);
	...
	protected function get_css_files(): array
	{
		...
		if ($_SESSION["preset"] != 1) {
			if ($streamit_options['streamit_layout_mode_options'] == 2) {
				$presetopt = 1;
				$presetopt1 = 1;
				$presetopt2 = 1;
			} else {
				...
				$presetopt2 = $_SESSION["preset"];
			}
		} else {
			/* if no preset varialbe found in url, use from theme options */
			$presetopt = isset($streamit_options['streamit_layout_mode_options']) ? $streamit_options['streamit_layout_mode_options'] : 1;
			$presetopt1 = isset($streamit_options['streamit_layout_mode_options']) ? $streamit_options['streamit_layout_mode_options'] : 1;
			$presetopt2 = isset($streamit_options['streamit_layout_mode_options']) ? $streamit_options['streamit_layout_mode_options'] : 1;
		}

		if (!isset($presetopt)) $presetopt = 1; /* in case first time install theme, no options found ...bootstrap*/
		if (!isset($presetopt1)) $presetopt1 = 1; /* in case first time install theme, no options found ...custom*/
		if (!isset($presetopt2)) $presetopt2 = 1; /* in case first time install theme, no options found...responsive */

		/* end layout style swticher option */
		$css_files = array(
			'slick-theme'			=> array(
				'file'   => 'vendor/slick/slick-theme.css',
				'global' => true,
			),
			'slick-animation'			=> array(
				'file'   => 'vendor/slick/slick-animation.css',
				'global' => true,
			),
			'slick'			=> array(
			...
			'bootstrap'			=> array(
				'file'   => 'vendor/bootstrap.min' . $presetopt . '.css',
				'global' => true,
			),
			'ionicons'			=> array(
			...
			'dark'			=> array(
				'file'   => 'vendor/dark.css',
				'global' => true,
			),
			'remixicon'			=> array(
				'file'   => 'vendor/remixicon.css',
				'global' => true,
			),
			'animate-min'			=> array(
			...
			'streamit-style'    => array(
				'file'             => 'streamit-style' . $presetopt1 . '.min.css',
				'global' => true,
			),
			'streamit-responsive'    => array(
				'file'             => 'streamit-responsive' . $presetopt2 . '.min.css',
				'global' => true,
			),
		);
	...
	protected function get_google_fonts(): array
	{
		...
		$google_fonts = array(
			'Heebo' => array('200', '300', '400', '500', '600', '700', '800'),
			'Roboto'     => array('400', '400i', '500', '500i', '700', '700i'),
		);
